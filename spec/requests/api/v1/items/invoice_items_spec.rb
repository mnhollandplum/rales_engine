...
describe 'Invoices Invoice Items API' do
  it "returns all invoice items for an Invoice " do
    ...
    expect(invoice_items["data"].first["id"]).to eq(invoice_item_3.id.to_s)
    expect(invoice_items["data"]).not_to include(invoice_item_2.id.to_s)
    expect(invoice_items["data"]).not_to include(invoice_item_1.id.to_s)
  end
end
